...
// --- 1. DATA SKILLS ---
const SKILLS = [
  // HAPUS HTML (Terlalu basic), GANTI DENGAN AI LIBRARY
...
{
    title: "Todotify Pro",
    // Deskripsi diperbaiki agar sesuai dengan Vanilla JS tapi tetap terdengar 'Advanced'
    ...
    // GANTI TAGS DARI REACT KE JS NATIVE
  ...
  },
  {
    title: "CS AI Hybrid",
    // REVISI DI SINI:
    // Ganti "NLP models" dengan "LLM/OpenAI".
    // "Human-in-the-loop" adalah istilah keren untuk fitur oper ke operator.
    ...
    tags: ["Laravel", "OpenAI API", "RAG", "Real-time"], // Tags diperbarui
    color: "green",
...
// --- COMPONENT: CUSTOM CURSOR (Optimized) ---
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    // Menggunakan requestAnimationFrame untuk performa lebih baik daripada event listener biasa
  ...
  return (
    <>
      ...
      <style>{`body { cursor: none; } a, button { cursor: pointer; }`}</style>
    </>
  )
}
...
const Keycap = memo(({ index, position, data, isActive, onActive, onInactive, animationState }) => {
  ...
  // Perubahan: smoothness diturunkan dari 2 ke 1 untuk performa
  return (
    <motion.group animate={animationState} variants={{
      hero: { x: 0, y: 0, z: 0, scale: 0.5 },
      profile: { x: -3, y: 1, z: -4, scale: 0.5 },
      skills: { x: position[0] * (isMobile ? 0.6 : 1), y: position[1] * (isMobile ? 0.6 : 1), z: position[2] * (isMobile ? 0.6 : 1), scale: 1 },
      projects: { x: position[0] * (isMobile ? 0.6 : 1), y: position[1] * (isMobile ? 0.6 : 1), z: position[2] * (isMobile ? 0.6 : 1), scale: 1 },
      guestbook: { x: position[0] * (isMobile ? 0.6 : 1), y: position[1] * (isMobile ? 0.6 : 1), z: position[2] * (isMobile ? 0.6 : 1), scale: 1 }
    }} transition={{ duration: 1.0 }}>
      <group ref={meshRef}>
        <mesh onPointerOver={(e) => { e.stopPropagation(); if(animationState === 'skills') onActive() }} onPointerOut={() => onInactive()} position={[0, isActive ? -0.15 : 0, 0]}>
...
const KeyboardChassis = memo(({ animationState }) => {
  ...
  return (
    <motion.group animate={animationState} variants={{
      ...
      skills: { scale: isMobile ? 0.7 : 1, opacity: 1 },
      projects: { scale: isMobile ? 0.7 : 1, opacity: 1 },
      guestbook: { scale: isMobile ? 0.7 : 1, opacity: 1 }
...
const BongoCat = memo(({ animationState }) => (
  <group position={[0, 0.63, 1.3]} rotation={[0, 0, -0.23]}>
    <Html transform occlude pointerEvents="none">
      <motionDom.div
         animate={{ opacity: (animationState === 'projects' || animationState === 'guestbook') ? 1 : 0, scale: (animationState === 'projects' || animationState === 'guestbook') ? 1 : 0 }}
...
const SceneContent = memo(({ setHoveredSkill, animationState }) => {
  ...
  return (
    <motion.group animate={animationState} variants={{
      ...
      skills: { scale: 1, x: isMobile ? 0 : 2.5, y: isMobile ? 1 : -1.0, z: 0, rotateX: 0.6, rotateY: isMobile ? 0 : -0.2, rotateZ: isMobile ? 0 : 0.1 },
      projects: { scale: 0.6, x: isMobile ? 0 : 3.5, y: -0.8, z: 0, rotateX: 0.1, rotateY: isMobile ? 0 : Math.PI - 0.4, rotateZ: 0 },
      guestbook: { scale: 0.7, x: isMobile ? 0 : -3.5, y: -0.5, z: 0, rotateX: 0.2, rotateY: 0.4, rotateZ: 0 } 
      ...
      {SKILLS.map((skill, index) => {
        ...
        return (
          <Keycap 
            ...
            onInactive={() => { if (activeKeyIndex === index) { setActiveKeyIndex(null); setHoveredSkill(null) }}}
...
// --- 4. VIEW 3D TERPISAH (KUNCI PERFORMA) ---
// Kita pisahkan Canvas ke komponen yang di-memoize.
// Component ini HANYA akan re-render jika `started`, `animationState`, atau `setHoveredSkill` berubah.
// Perubahan scrollY di App TIDAK akan men-trigger render ulang di sini.
...
function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Logika state sederhana
      if (y > viewportHeight * 0.5) newState = 'profile' // Trigger lebih cepat
  ...
  return (
    <div className="relative w-full bg-[#050505] min-h-[750vh] font-sans">
      ...
      {/* Render 3D Scene terpisah agar tidak re-render saat scrollY berubah */}
  ...
  return (
    <div className="absolute top-0 left-0 w-full z-10 pointer-events-none">
      ...
      <section id="hero" className="h-screen w-full flex flex-col justify-center items-center text-center px-4">
        <motionDom.div animate={{ opacity: showHero ? 1 : 0, y: showHero ? 0 : -30 }} transition={{ duration: 0.8 }}>
          ...
          <p className="text-xl md:text-2xl text-gray-400 font-light max-w-2xl mx-auto leading-relaxed">
            Turning ideas <span className="text-cyan-400 font-medium">ideas</span> <br/>
      ...
      <section id="skills" className="h-[200vh] w-full relative">
        <div className={`sticky top-0 h-screen w-full flex items-center px-6 md:px-20 transition-all duration-700 ${showSkills ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="w-full max-w-lg relative z-10 pointer-events-none">
             ...
             {hoveredSkill ? (
               <motionDom.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} key={hoveredSkill.name} className="bg-zinc-900/90 backdrop-blur-2xl border border-white/10 p-8 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden mt-6">
                  ...
                  <div className="h-1.5 w-16 rounded-full mb-6" style={{ backgroundColor: hoveredSkill.color }}></div>
...
const GuestBookComponent = () => {
  ...
  // 2. Ambil Pesan Realtime dari Firestore
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, "guestbook"), {
        text: input,
        name: user.displayName, // Nama dari Google
        uid: user.uid,
        photo: user.photoURL, // Foto Profil Google
  ...
  return (
    <div className="flex flex-col h-[350px]">
      {/* HEADER LOGOUT (Muncul jika login) */}
      {user && (
